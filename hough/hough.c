...
struct coordonate *coor(double *lignes, int *size, SDL_Surface *surface)
{
	...
	average(0, m, k, surface->w, surface->h);
	average(1, m, k, surface->w, surface->h);
...
void remove_coor(int i, struct coordonate *m)
{
	...
	/*for (; i<(*size)-1; i++)
	{
		...
		(*m)[i].y1 = (*m)[i+1].y1;
		(*m)[i].y2 = (*m)[i+1].y2;
		(*m)[i].lignes = (*m)[i+1].lignes;

	}

	(*size)--;
	*m = realloc(*m, (*size) * sizeof(struct coordonate));
	*/
}

void insert_coor(int i, struct coordonate **m, int size, struct coordonate *cor)
{
	(*m) = realloc(*m, (size+1)*sizeof(struct coordonate));
...
void average(int li, struct coordonate *m, int size, int w, int h)
{
	int k =0;
	int p =0;
	...
	for (int i =1; i<size; i++)
	{
		...
		if (m[i].lignes == li)
		{
			...
			if (li == 0)
			{
				...
				/*
				else if (m[i].x1-p>w/9)
				{
					struct coordonate *cor = malloc(sizeof(struct coordonate));
					cor->x1 = p +w/10;
					cor->x2 = p + w/10;
					cor->y1 = m[i].y1;
					cor->y2 = m[i].y2;
					insert_coor(i, &m, size, cor);
					size++;
					//free(cor);
